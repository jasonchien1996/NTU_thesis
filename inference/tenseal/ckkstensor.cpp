...
shared_ptr<CKKSTensor> CKKSTensor::matmul_plain_inplace(
    ...
    task_t worker_func = [&](size_t start, size_t end) -> bool {
        ...
        for (size_t i = start; i < end; i++) {
            ...
            for (auto &it : to_sum) {
                if (ct.parms_id() == it.parms_id()) continue;
        
                size_t idx = get_chain_index(it);
                if (idx < ct_idx) {
                    ct_idx = idx;
                    ct = it;
                }
            }
            this->auto_same_mod(ct, to_sum);
...
//_data is a 3D tensor
shared_ptr<CKKSTensor> CKKSTensor::pool_inplace(
    const vector<vector<size_t>>& locations,
    const vector<PlainTensor<double>>& kernels) {
    
    if(shape().size()!=3) throw invalid_argument("Error: tensor shape must be 3-dimensional!");
        
    vector<Ciphertext> new_data;
    new_data.resize(locations.size());
        
    task_t worker_func = [&](size_t start, size_t end) -> bool {
        for (size_t i = start; i < end; i++) {
            const vector<size_t>& location = locations[i];
            const PlainTensor<double>& kernel = kernels[location[0]];
            
            vector<size_t> shape = kernel.shape();
            size_t k_c = shape[0], k_h = shape[1], k_w = shape[2];
            
            vector<Ciphertext> to_sum;
            to_sum.resize(k_h*k_w);
            
            size_t j = -1;
            for(size_t c = 0; c < k_c; c++){
                if(kernel.at({c, 0, 0}) == 0) continue;
                for(size_t h = 0; h < k_h; h++){
                    for(size_t w = 0; w < k_w; w++){
                        Plaintext pt;
                        to_sum[++j] = this->_data.at({c, location[1]+h, location[2]+w});
                    }
                }
            }
            Ciphertext acc(*this->tenseal_context()->seal_context(), to_sum[0].parms_id());
            this->tenseal_context()->evaluator->add_many(to_sum, acc);
            new_data[i] = acc;
        }
        return true;
    };

    this->dispatch_jobs(worker_func, locations.size());
    
    this->_data = TensorStorage(new_data);

    return shared_from_this();
}

//_data is a 3D tensor
shared_ptr<CKKSTensor> CKKSTensor::conv_fusion_inplace(
    const vector<vector<size_t>>& locations,
    const vector<PlainTensor<double>>& kernels) {
    ...
    task_t worker_func = [&](size_t start, size_t end) -> bool {        
        for (size_t i = start; i < end; i++) {
            ...
            vector<Ciphertext> to_sum;
            to_sum.resize(kernel.flat_size());
            ...
            for(size_t c = 0; c < k_c; c++){
                for(size_t h = 0; h < k_h; h++){
                    for(size_t w = 0; w < k_w; w++){
                        ...
                        this->tenseal_context()->encode<CKKSEncoder>(kernel.at({c,h,w}), pt, this->_init_scale);
                        this->perform_plain_op(to_sum[j], pt, OP::MUL);
                    }
                }
            }
...
void CKKSTensor::set_same_mod(){
    ...
    auto& ct = *(this->_data.begin());
    size_t ct_idx = get_chain_index(ct);
    for (auto it = this->_data.begin()+1; it != this->_data.end(); ++it) {
        if (ct.parms_id() == (*it).parms_id()) continue;
